...
static int arpsc_cmp_ifaces(const char **__restrict__ ifacearr, const char *__restrict__ iface) {
...
void arpsc_print_arp_format(const char *__restrict__ pformat, iFaceAttr *__restrict__ iface,
							ARPRESPPACK *__restrict__ replay, unsigned int time) {
	...
	do {
		...
		case 'r':
			...
			break;
		case 'M':
			printf("smac='%x:%x:%x:%x:%x:%x'%c", replay->sender_mac[0], replay->sender_mac[1], replay->sender_mac[2],
				   replay->sender_mac[3], replay->sender_mac[4], replay->sender_mac[5], endof);
			break;
		case 'm':
			printf("tmac='%x:%x:%x:%x:%x:%x'%c", replay->target_mac[0], replay->target_mac[1], replay->target_mac[2],
				   replay->target_mac[3], replay->target_mac[4], replay->target_mac[5], endof);
			break;
		case 'h':
			/*	*/
